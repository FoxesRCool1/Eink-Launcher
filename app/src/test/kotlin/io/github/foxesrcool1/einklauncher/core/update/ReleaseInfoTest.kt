...
class ReleaseParserTest {
    ...
    fun `the list GitHub sends is read`() {
        ...
        assertEquals("Eink Launcher v0.2.0", newest.title)
        assertEquals("## 0.2.0\n\n- A fix.", newest.notes)
        assertEquals(false, newest.prerelease)
        assertEquals(4, newest.assets.size)

        val asset = newest.assets.first { it.name == "eink-launcher-v0.2.0-viwoods.apk" }
    ...
    fun `a file that is still uploading is left out`() {
        ...
        assertEquals(listOf("eink-launcher-v0.1.1-viwoods-debug.apk"), old.assets.map { it.name })
    }
...
class UpdateAssetsTest {
    ...
    fun `each build takes only its own kind of file`() {
        val newest = releases.first { it.tag == "v0.2.0" }
        assertEquals("eink-launcher-v0.2.0-viwoods.apk", UpdateAssets.pick(newest, "viwoods", debug = false)?.name)
        assertEquals("eink-launcher-v0.2.0-viwoods-debug.apk", UpdateAssets.pick(newest, "viwoods", debug = true)?.name)
        assertEquals("eink-launcher-v0.2.0-generic.apk", UpdateAssets.pick(newest, "generic", debug = false)?.name)
    ...
    fun releasesJson(debugSha: String = "b".repeat(64), debugSize: Long = 2000): String = """
        [
          ...
          {"tag_name":"nightly","name":"nightly","draft":false,"prerelease":false,"body":"","assets":[]},
          {
            ...
            "assets":[
              {"name":"eink-launcher-v0.1.1-viwoods-debug.apk","size":10,"state":"uploaded",
               "url":"https://api.github.com/repos/o/r/releases/assets/1","digest":null},
              {"name":"eink-launcher-v0.1.1-generic-debug.apk","size":0,"state":"starter",
               "url":"https://api.github.com/repos/o/r/releases/assets/2"}
            ]
          },
          {
            "tag_name":"v0.2.0","name":"Eink Launcher v0.2.0","draft":false,"prerelease":false,
            "body":"## 0.2.0\n\n- A fix.",
            "assets":[
              {"name":"eink-launcher-v0.2.0-viwoods.apk","size":1000,"state":"uploaded",
               "url":"https://api.github.com/repos/o/r/releases/assets/11",
               "digest":"sha256:${"A".repeat(64)}"},
              {"name":"eink-launcher-v0.2.0-generic.apk","size":1000,"state":"uploaded",
               "url":"https://api.github.com/repos/o/r/releases/assets/12","digest":"sha256:${"c".repeat(64)}"},
              {"name":"eink-launcher-v0.2.0-viwoods-debug.apk","size":$debugSize,"state":"uploaded",
