...
class InkCanvasViewTest {
    ...
    fun `a full page paints in reasonable time and exports`() {
        ...
        val note = InkNote(template = PageTemplate.DotGrid, pages = listOf(io.github.foxesrcool1.einklauncher.core.ink.InkPageData(page)))
        val png = InkExport.pagePng(note, note.pages[0], InkExport.PREVIEW_SCALE)
        assertTrue(png.size > 1_000)
        java.io.File(io.github.foxesrcool1.einklauncher.support.Screenshots.pathFor("ink_full_page_preview")).apply {
            parentFile?.mkdirs()
            writeBytes(png)
        }
